...
## 当前版本状态
- 最近已发布稳定节点：`6.0`（Git tag：`research-v6.0`）
- 当前工作区主线与 `research-v6.0` 对齐；该版本在 `research-v5.1` 的结构与口径收口基础上，完成正式论文输出链收紧：只保留 baseline / full improved 对比、业务/切换/负载层 KPI、每星原始状态 trace、轨迹/HTML 示意材料，并把 PHY/SINR/TBler 从正式结果表降为诊断入口
...
- `hexCellRadiusKm = 20`
- `poissonLambda = 1.5`（仅 `ueLayoutType=poisson-3ring` 可选入口使用）
- `maxUePerCell = 5`（仅 `ueLayoutType=poisson-3ring` 可选入口使用）
- `ueLayoutRandomSeed = 42`（仅 `ueLayoutType=poisson-3ring` 可选入口使用）
- `enforceBeamExclusionRing = true`
- `beamExclusionMode = ring`（默认保持“同格 + 一圈邻格”锚点排他；诊断可选：`overlap-only`、`off`）
- `beamExclusionCandidateK = 64`
- `realLinkGateMode = beam-and-anchor`（默认保持“主波束 + anchor 小区”双门控；诊断可选：`beam-only`、`off`）
...
- 当前新增 `ueLayoutType = poisson-3ring` 作为可选场景入口：在中心及两圈共 `19` 个 hex cell 内按截断泊松权重分配 `UE`，并在各 cell 内随机撒点；该入口用于负载空间分布对照，不替代正式 `seven-cell` baseline
- 当前默认波束锚点启用一圈排他：一个卫星波束占用某个 hex 中心后，该中心及其周围一圈邻区都不会再被其它卫星选作最终落点
- 当前新增可选 `beamExclusionMode` 诊断入口：
  - `ring`：保持默认“同格 + 一圈邻格”排他
  - `overlap-only`：只禁止多个卫星占同一个 anchor cell，允许落在一圈邻格
  - `off`：完全关闭 anchor 排他
...
- 当前新增可选 `realLinkGateMode` 诊断入口：
  - `beam-and-anchor`：保持默认“主波束 + anchor 小区”双门控
  - `beam-only`：只保留主波束门控，放宽 anchor 小区约束
  - `off`：完全关闭真实接入/切换候选 gate
...
- 当前默认输出：
  ...
  - `hex_grid_cells.csv`
  - `hex_grid_cells.html`（当 `runGridHtmlScript = true` 且至少启用一个卫星轨迹 overlay）
  - `ue_layout.csv`
...
- 当前常用分析脚本：
  ...
  - `scratch/plotting/summarize_thesis_results.py`
    - 读取 `scratch/results/formal/{baseline,improved}/seed-*`，输出 `run_summary.csv`、`paper_kpi_summary.csv` 与 `paper_kpi_comparison.csv`；不读取 PHY/SINR/TBler 文件
...
## 当前收口结论
- `research-v6.0` 的核心收口是正式输出路径，而不是更换默认场景：baseline / full improved 仍回到同一固定 `2x4`、`25 UE`、`seven-cell` 口径下比较
...
- 新近补充的 `19 UE + 40 s + no-exclusion` 结果可作为一条场景对照：
  ...
  - `E2E delay = 56.733 ms`
  - `packet loss = 25.584%`
  - `mean SINR = 7.912 dB`（历史诊断口径，后续不进入正式主表）
  - completed HO `27`
  - ping-pong `0`
