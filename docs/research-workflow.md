...
- `4.1` 提供当前默认 baseline / improved 语义：
...
## 分支规则
- `main`：保持可回溯、可运行的主线基线。
- `feature/v4.1-xxx`：功能开发，例如切换触发、邻区门控、日志重构。
- `experiment/v4.1-xxx`：实验性方案验证，例如新 TTT、hysteresis、目标选择策略。
- `fix/v4.1-xxx`：缺陷修复。
- `docs/v4.1-xxx`：文档、实验记录和说明更新。
- 当前优先推荐的功能分支主题：
  - `feature/v4.1-a3-baseline`
  - `feature/v4.1-load-aware-handover`
  - `feature/v4.1-neighbor-gating`

## 提交信息规则
- 主阶段提交推荐格式：`type(v4.1): 简短说明`
- 稳定节点收口提交可使用：`type(v4.1.x): 简短说明`
- 常用 `type`：
  - `feat`
  - `fix`
  - `exp`
  ...
  - `chore`
- 示例：
  - `feat(v4.1): refine source-load-aware target selection`
  - `exp(v4.1): compare visibility and load-aware settings`
  - `fix(v4.1): avoid writing outputs to repo root`
  - `chore(v4.1): snapshot joint signal-visibility-load release`
