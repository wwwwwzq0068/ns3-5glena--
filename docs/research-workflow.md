...
## 版本定义
- 当前研究主阶段定义为 `6.2`。
- 最近稳定节点定位为 `6.2`（建议 Git tag：`research-v6.2`）。
...
- `research-v6.2` 是在 `research-v6.1` 场景口径不变基础上的主线合并发布包：完成 baseline 文件瘦身、切换决策 helper 拆分和文档版本收口。
- 这里的 `6.2 / 6.2.x` 指本仓库中的“毕设/研究工作版本”，不是 ns-3 框架本身的版本号。
...
- `6.2` 继承当前默认 baseline / improved 语义：
...
## 分支规则
- `main`：保持可回溯、可运行的主线基线。
- `feature/v6.2-xxx`：功能开发，例如切换触发、邻区门控、日志重构。
- `experiment/v6.2-xxx`：实验性方案验证，例如新 TTT、hysteresis、目标选择策略。
- `fix/v6.2-xxx`：缺陷修复。
- `docs/v6.2-xxx`：文档、实验记录和说明更新。
- 当前优先推荐的功能分支主题：
  - `feature/v6.2-a3-baseline`
  - `feature/v6.2-load-aware-handover`
  - `feature/v6.2-neighbor-gating`

## 提交信息规则
- 主阶段提交推荐格式：`type(v6.2): 简短说明`
- 稳定节点收口提交可使用：`type(v6.2.x): 简短说明`
- 常用 `type`：
  - `feat`
  - `fix`
  - `exp`
  ...
  - `chore`
- 示例：
  - `feat(v6.2): refine source-load-aware target selection`
  - `exp(v6.2): compare visibility and load-aware settings`
  - `fix(v6.2): avoid writing outputs to repo root`
  - `chore(v6.2): snapshot thesis scenario release`
...
## 版本推进规则
- `6.2` 代表当前论文正式场景与代码瘦身收口阶段，不因为每次小改动就升大版本。
- `6.2` 代表当前主阶段下的稳定节点。
