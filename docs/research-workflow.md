...
- 当前研究主阶段定义为 `3.0`。
- 当前已收敛的稳定节点定义为 `3.0.1`。
- 这里的 `3.0 / 3.0.1` 指本仓库中的“毕设/研究工作版本”，不是 ns-3 框架本身的版本号。
...
- 当前优先推荐的功能分支主题：
  - `feature/v3.0-a3-baseline`
  - `feature/v3.0-load-aware-handover`
  - `feature/v3.0-neighbor-gating`

## 提交信息规则
- 主阶段提交推荐格式：`type(v3.0): 简短说明`
- 稳定节点收口提交可使用：`type(v3.0.1): 简短说明`
- 常用 `type`：
  - `feat`
  - `fix`
  - `exp`
...
- 示例：
  ...
  - `chore(v3.0.1): stabilize baseline runtime and performance`
...
## 当前执行原则
- 先稳住 `3.0` 基线，再围绕传统 A3 baseline 和改进策略开展对比设计。
