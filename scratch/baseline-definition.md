...
说明：
- 这套参数代表当前工作区的默认 baseline 口径；最近已发布稳定节点为 `research-v6.0`
...
- 当前还支持显式设置 `ueLayoutType=poisson-3ring` 做空间负载分布对照：在中心及两圈共 `19` 个 hex cell 内按截断泊松权重生成 `UE` 分配，并在各 cell 内随机撒点；当前实现保持总数等于 `ueNum`，以避免破坏现有节点、业务和统计管线
- 当前默认仍保持 `anchorGridHexRadiusKm = hexCellRadiusKm`，并启用波束锚点一圈排他：其它卫星不能把最终落点选到已占锚点或其一圈邻区内
- 当前还支持显式设置 `beamExclusionMode=overlap-only` 做中间口径诊断：允许不同卫星的 anchor 落在一圈邻格，但仍禁止多个卫星占同一个 anchor cell；`beamExclusionMode=off` 则完全关闭 anchor 排他
...
- 当前稳定默认真实 NR PHY 为 `gNB b00-custom + UE three-gpp`，阵列规模 `gNB 12x12`、`UE 1x2`，该口径自 `v4.3` 起引入，当前 `research-v6.0` 继续沿用；正式论文结果不再把 PHY/SINR/TBler 放入主表
...
- 当前还支持显式设置 `realLinkGateMode=beam-only` 做中间口径诊断：只保留主波束门控、放宽 anchor 小区 gate；`realLinkGateMode=off` 则完全关闭真实接入/切换候选 gate
