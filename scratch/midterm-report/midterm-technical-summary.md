...
### 2.2 当前阶段定位
当前研究仓库最近已发布稳定节点为 `4.2`（Git tag：`research-v4.2`）。这里的 `4.2.x` 是研究工作版本，不是 ns-3 框架版本；底层仿真框架仍为 `ns-3.46`。当前工作区若继续补充图表、PPT 或结果总结，应视为 `4.2` 之后的主阶段内迭代。
...
其中 `loadScore` 当前采用更平滑的容量压力近似，避免在每星 `5 UE` 容量口径下过早饱和。

同时，当前 improved 已开始把基于轨道几何的剩余可见时间作为目标选择辅助量接入，并在源站接近拥塞时进一步增强负载导向；在最新主线中，还增加了“同一目标必须持续领先一段时间”和“最佳候选联合分数必须明显优于当前服务星”的额外约束，但仍保持统一 `MeasurementReport` 触发入口不变。
