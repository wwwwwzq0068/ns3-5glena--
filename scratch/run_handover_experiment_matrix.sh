...
Options:
  --list                    List the retained thesis experiments.
  --run ID                  Run one experiment ID: B00 or I31.
  --group NAME              Run a predefined group.
  --all                     Run the retained formal compare group.
...
list_experiments() {
    cat <<'EOF'
Retained Thesis Experiment Matrix
  B00  baseline control     handoverMode=baseline hoTttMs=160 hoHysteresisDb=2.0
  I31  improved reference   handoverMode=improved hoTttMs=160 hoHysteresisDb=2.0
                             improvedSignalWeight=0.7 improvedLoadWeight=0.3
                             improvedMinStableLeadTimeSeconds=0.12 improvedMinJointScoreMargin=0.03
...
if [ "$RUN_ALL" = "1" ]; then
    SELECTED_IDS="$SELECTED_IDS B00 I31"
fi
